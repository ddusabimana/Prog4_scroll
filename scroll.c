/*
Author: Daniel Dusabimana and Samuel Capotosto
*/
#include <stdio.h>
#include <stdlib.h>
#include <curses.h>
...
#define IGNORE_TICKS		(0)					//.Ignore first 'n' Timer Ticks
...
void installHandlers() //.Install timer_handler as the signal handler for SIGVTALRM
{
	if(signal(SIGALRM, scrollHandler) == SIG_ERR) { //.(void (*)(int))
		perror("\nError : Unable to catch SIGVTALRM !\n");
		exitProgram(EXIT_FAILURE);
	}
